# Arch-Guns
All Archwing-mounted primary weapons usable in space and with Gravimag on land. Includes damage types, critical stats, status chance, and crafting info.

## Arbucep
Nokko’s signature archgun fires six homing missiles, each bearing a payload of one of the six combined elements which detonate in an area upon impact.

### Wiki
Weapon
Nokko's signature archgun fires six homing missiles, each bearing a payload of one of the six combined elements which detonate in an area upon impact.
Arbucep is Nokko's Signature Arch-gun. This mushroom-shaped launcher simultaneously fires six rockets, one from each of the secondary elements (Blast, Corrosive, Gas, Magnetic, Radiation, and Viral). Shooting at Nokko's Stinkbrain or Brightbonnet causes them to release their pulses and become empowered.
Characteristics
This weapon deals primarily Blast, Corrosive, Gas, Magnetic, Radiation, and Viral damage.
Multishot increases weapon damage instead of creating additional projectiles.
Damage bonus is multiplicative to other sources of damage.
Arbucep's main and component blueprints are acquired from Deepmines Bounties as an uncommon and rare drop. Access requires completion of The New War.
Alternatively Arbucep's blueprints can be purchased from Nightcap in Fortuna with Fergolyte from Deepmines Bounties. Each component blueprint requires 150 and the main blueprint requires 220, totaling 670 Fergolytes for the whole set. Players must be at Rank 3 - Seeker with Nightcap in order to purchase.
Crafting
The Arbucep appears to be a fungal growth over an existing, presumably corpus-made, heavy weapon.
**Type:** Arch-Gun | **Riven Disposition:** 1

### 1st Attack
**Damage:** blast: 16
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 1st Attack Radial Attack
**Damage:** blast: 114
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 2nd Attack
**Damage:** corrosive: 16
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 2nd Attack Radial Attack
**Damage:** corrosive: 114
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 3rd Attack
**Damage:** gas: 16
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 3rd Attack Radial Attack
**Damage:** gas: 114
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 4th Attack
**Damage:** magnetic: 16
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 4th Attack Radial Attack
**Damage:** magnetic: 114
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 5th Attack
**Damage:** radiation: 16
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 5th Attack Radial Attack
**Damage:** radiation: 114
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 6th Attack
**Damage:** viral: 16
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### 6th Attack Radial Attack
**Damage:** viral: 114
Crit Chance: 10.00% | Crit Multiplier: 2.9x | Status Chance: 34.90% | Fire Rate: 1.5

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Nokko: Corporate Restructuring (Steel Path), Rotation C (10.00%); Nokko: Corporate Restructuring Rewards, Rotation C (10.00%); Nokko: Corporate Restructuring Rewards, Rotation B (2.50%); Nokko: Corporate Restructuring (Steel Path), Rotation B (5.00%) |
| Blueprint | 1 | Nokko: Corporate Restructuring Rewards, Rotation C (5.00%); Nokko: Critter Liberation Rewards, Rotation C (5.00%); Nokko: Weed The Gardens Rewards, Rotation C (5.00%); Nokko: Corporate Restructuring (Steel Path), Rotation C (8.00%); Nokko: Critter Liberation Rewards (Steel Path), Rotation C (8.00%); Nokko: Weed The Gardens Rewards (Steel Path), Rotation C (8.00%) |
| Dull Button | 5 | — |
| Receiver | 1 | Nokko: Critter Liberation Rewards (Steel Path), Rotation C (10.00%); Nokko: Critter Liberation Rewards, Rotation C (10.00%); Nokko: Critter Liberation Rewards, Rotation B (2.50%); Nokko: Critter Liberation Rewards (Steel Path), Rotation B (5.00%) |
| Stock | 1 | Nokko: Weed The Gardens Rewards (Steel Path), Rotation C (10.00%); Nokko: Weed The Gardens Rewards, Rotation C (10.00%); Nokko: Weed The Gardens Rewards, Rotation B (2.50%); Nokko: Weed The Gardens Rewards (Steel Path), Rotation B (5.00%) |

## Cortege
An ancient weapon designed by the Entrati for use by their Necramechs. Primary fire siphons life essence from the target to fuel a devastating secondary fire. A heavy flamethrower with surprising range. Secondary fire launches three projectiles in a fan pattern that explode, leaving a damaging area of effect for a short duration.

### Wiki
Orokin
An ancient weapon designed by the Entrati for use by their Necramechs. Primary fire siphons life essence from the target to fuel a devastating alt fire. A heavy flamethrower with surprising range. Alt fire launches three projectiles in a fan pattern that explode, leaving a damaging area of effect for a short duration.
The Cortege is an Orokin Era weapon created by the Entrati, a beam Archgun that builds charges on kills to allow use of the Alternate Fire, a trio of grenades that leaves lingering flames.
Characteristics
This weapon deals primarily Heat damage on its primary fire and Blast damage on its Alternate Fire.
Primary fire releases a flamethrower-like beam.
Pinpoint accuracy.
Consumes 0.5 ammo per tick of damage.
Beam has a thickness of ~1 meter.
Beam deals radial damage varying from of 1 to 4 meters in a cone shape.
The Atmospheric beam has innate Infinite Body Punch Through.
No Damage Falloff in Archwing Mode.
Getting 5 kills with the Cortege's primary fire will unlock an Alternate Fire that releases a trio of grenades that explodes in an 4 meter radius on impacting a surface or when remotely detonated with Alternate Fire, leaving lingering flames that deal continuous damage for 11 seconds.
Initial hit and explosion apply status separately.
Explosion does not need direct line of sight to deal damage and will penetrate walls.
Consumes no ammo.
After using Alternate Fire, additional kills are needed to recharge the grenade barrage.
Innate Madurai polarity.
Primary Fire has the second highest status chance of all Archguns, behind Arquebex/Mausolon.
**Type:** Arch-Gun | **Mastery Req:** 14 | **Riven Disposition:** 3

### Held
**Damage:** heat: 90
Crit Chance: 20.00% | Crit Multiplier: 1.9x | Status Chance: 40.00% | Fire Rate: 12

### Grenade Impact
**Damage:** impact: 10
Crit Chance: 30.00% | Crit Multiplier: 2.4x | Status Chance: 16.70% | Fire Rate: 0.25

### Radial Attack
**Damage:** blast: 1000
Crit Chance: 30.00% | Crit Multiplier: 2.4x | Status Chance: 16.70% | Fire Rate: 0.25

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | NecraLoid (Loid), Clearance Odima (100.00%) |
| Cortege Barrel | 1 | — |
| Cortege Receiver | 1 | — |
| Cortege Stock | 1 | — |

## Corvas
When fully charged, this flak cannon delivers a devastating shot. Perfect for taking down fast-moving interceptors.

### Acquisition
The Corvas's blueprint can be purchased from the Market. Its parts can be purchased from various Syndicates. All parts, except for the blueprint, are tradeable.

### Wiki
Tenno
When fully charged, this flak-cannon delivers a devastating shot. Perfect for taking down fast-moving interceptors.
The Corvas flak cannon is a heavy Archgun with shotgun properties. It can be rapidly fired to saturate an area with pellets or charged to release a more powerful scattershot.
Characteristics
This weapon deals primarily Heat damage.
Shots have a guaranteed Impact proc.
Innate Naramon polarity.
Second highest critical chance and critical multiplier of all Archguns, behind Velocitus.
Atmosphere:
Tight pellet spread. Pellet spread can hit multiple targets per shot.
Good ammo efficiency.
Archwing:
Has linear damage falloff from 100% to ?% from 100m to 200m target distance (distances are affected by Projectile Speed).
Long reload delay of 3 seconds.
Extremly slow ammo regeneration rate.
Atmosphere:
Has linear damage falloff from 100% to 50% from 60m to 120m target distance (distances are affected Projectile Speed).
Fully charged shots cause a heavy recoiling effect that breaks zoom and knockback.
Low overall status chance.
Short maximum effective range even with charged shot.
Corvas Prime
**Notes:**
In Archwing mode, Corvas fires a wide energy projectile, similar to the Arca Plasmor. In Atmospheric mode, it functions like a shotgun, and fires hit-scan pellets.
**Tips:**
As a shotgun-like weapon, the Corvas is best used when engaging enemies at close range, where its scattershot spread can be used to deal massive damage against enemy units.
Charging this weapon is not advised, as after modding for fire rate, the added charge time reduces total dps more than the higher base damage compensates for it. In atmosphere, the fully-charged stagger further feeds the disparity.
**Type:** Arch-Gun | **Mastery Req:** 1 | **Riven Disposition:** 4

### Uncharged Shot
**Damage:** impact: 200, heat: 240
Crit Chance: 40.00% | Crit Multiplier: 2.6x | Status Chance: 13.00% | Fire Rate: 2

### Charged Shot
**Damage:** impact: 400, heat: 480
Crit Chance: 40.00% | Crit Multiplier: 3x | Status Chance: 13.00% | Fire Rate: 2

### Crafting
| Component | Count | Drops |
|---|---|---|
| Argon Crystal | 2 | — |
| Barrel | 1 | Arbiters of Hexis, Authentic (100.00%) |
| Blueprint | 1 | — |
| Receiver | 1 | Cephalon Suda, Intriguing (100.00%) |
| Stock | 1 | Steel Meridian, Valiant (100.00%) |

## Corvas Prime
Flaunting ceremonial beauty, this arch gun flak cannon is even more devastating than its standard issue counterpart.

### Wiki
Weapons
Flaunting ceremonial beauty, this arch-gun flak-cannon is even more devastating than its standard issue counterpart.
Corvas Prime is the Primed variant of the Corvas Archgun flak-cannon, sporting improvements to its Damage, Critical Chance, Status Chance, Reload Speed, and Atmospheric Damage Falloff, with some expense to Magazine Size. Instead of firing shotgun-like rounds, the Corvas Prime shoots single, large Arca Plasmor-like projectiles whether it's used in space or solid ground. It was released alongside Garuda Prime and Nagantaka Prime.
Characteristics
This weapon deals primarily Heat damage.
Shots have a guaranteed Impact proc.
Two innate Madurai and a single Naramon polarity.
Corvas
**Notes:**
The projectile fired is similar to the Arca Plasmor, a large projectile that can hit lines of enemies if the center of the projectile doesn't directly impact enemies, allowing the surrounding wave to hit anyone within its small radius as it travels.
Equipping Sabot Rounds can compensate for instances of a direct hit.
**Tips:**
With its high damage and ability to hit the head of enemies if aimed above enemy heads, Archgun Ace can be easy to proc giving you bonus fire/charge rate as well as reducing the delay before the ammo begins to regenerate. While the buff cannot refresh, as long as the buff is active when you stop firing the delay reduction will still be applied.
Charged shots will stagger the player in Heavy Weapon mode, so normal shots are recommended instead while saving the higher powered charged shots for Necramechs as they are unaffected by staggering effects.
When used against Profit-Taker, loading Corvas Prime with Sabot Rounds allows it to take out two or even three legs at the same time with well angled close range shots.
**Trivia:**
Corvas Prime is the first Primed Archgun.
**Type:** Arch-Gun | **Mastery Req:** 14 | **Riven Disposition:** 3

### Uncharged Shot
**Damage:** impact: 230, heat: 330
Crit Chance: 44.00% | Crit Multiplier: 3x | Status Chance: 20.00% | Fire Rate: 2

### Charged Shot
**Damage:** impact: 430, heat: 530
Crit Chance: 56.00% | Crit Multiplier: 3x | Status Chance: 20.00% | Fire Rate: 2

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Meso C9 Relic (2.00%); Neo C2 Relic (2.00%); Neo C4 Relic (2.00%); Neo C5 Relic (2.00%) |
| Blueprint | 1 | Axi K7 Relic (11.00%); Lith W3 Relic (11.00%); Meso B8 Relic (11.00%); Meso H8 Relic (11.00%); Neo D5 Relic (11.00%); Neo L2 Relic (11.00%) |
| Orokin Cell | 10 | — |
| Receiver | 1 | Axi G10 Relic (25.33%); Axi K8 Relic (25.33%); Axi T10 Relic (25.33%); Lith D6 Relic (25.33%); Lith G8 Relic (25.33%); Meso A4 Relic (25.33%); Meso G4 Relic (25.33%); Neo D7 Relic (25.33%) |
| Stock | 1 | Axi G8 Relic (25.33%); Lith K10 Relic (25.33%); Lith S12 Relic (25.33%); Meso G5 Relic (25.33%); Neo C3 Relic (25.33%); Neo K8 Relic (25.33%) |

## Cyngas
Unload deadly accurate bursts of mayhem.

### Acquisition
The Cyngas's blueprint can be purchased from the Market. Its parts can be purchased from various Syndicates. All parts, except for the blueprint, are tradeable.

### Wiki
Tenno
Unload deadly accurate bursts of mayhem.
Cyngas is an Archgun Auto Burst-fire rifle, with very high accuracy, status chance, and reload speed.
Characteristics
This weapon deals an almost even amount of Slash, Puncture and Impact damage.
Good ammo efficiency, generating relatively small cooldown for the Archgun Deployer for every second fired.
Short reload delay of 0.25 seconds in Archwing.
Fastest reload speed of all Archguns in Atmospheric mode.
**Trivia:**
Cyngas appears to be derived from Cygnus, a constellation visible in the Northern Hemisphere that resembles a swan. The "fins" near the Archgun's upper handle resemble the wings of the constellation in question. The word "cygnus" itself the Latin word for "swan".
The Cyngas is the first Archwing Auto Burst-fire weapon, and the first Auto Burst trigger weapon in the game overall.
Despite having an Auto Burst trigger in Archwing mode, in Atmosphere mode the weapon switches to a regular Burst-fire trigger.
Shares an aesthetic with the Agkuza Archwing sword.
The Cyngas is the only archwing weapon that has a part that costs 25.000 Syndicate Standing, all other parts are the regular ammount 20.000.
**Type:** Arch-Gun | **Mastery Req:** 4 | **Riven Disposition:** 4

### Normal Attack
**Damage:** impact: 39.6, slash: 40.8, puncture: 39.6
Crit Chance: 20.00% | Crit Multiplier: 2.2x | Status Chance: 30.00% | Fire Rate: 6.67

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Arbiters of Hexis, Lawful (100.00%) |
| Blueprint | 1 | — |
| Receiver | 1 | Cephalon Suda, Intelligent (100.00%) |
| Stock | 1 | The Perrin Sequence, Executive (100.00%) |
| Tellurium | 2 | — |

## Dual Decurion
Delivering twin streams of highly accurate, rapid-fire ordnance, the Decurion are specifically designed for combat in the vacuum of space.

### Acquisition
The Dual Decurion's blueprint can be purchased from the Market. Its parts can be purchased from various Syndicates. All parts, except for the blueprint, are tradeable.

### Wiki
Tenno
Delivering twin streams of highly accurate, rapid-fire ordnance, the Decurion are specifically designed for combat in the vacuum of space.
The Dual Decurion are twin Archgun rifles boasting high critical chance and high reload speed.
Characteristics
This weapon deals primarily Impact damage.
Good ammo efficiency, generating relatively small cooldown for the Archgun Deployer for every second fired.
Short reload delay of 0.25 seconds in Archwing mode.
Tied with Prisma Dual Decurions for the second fastest reload speed of all Archguns in Atmospheric mode, behind Cyngas.
Has notable recoil which throws off accuracy at medium-long range shooting.
Prisma Dual Decurions
**Notes:**
Despite the appearance of its shots, the Dual Decurion is a hitscan weapon by default.
Starting from Update 27, all hitscan Archwing weapons are converted to projectile when in space. Such weapons (Dual Decurion included) still retain their hitscan properties while deployed in ground missions with an Archgun Deployer.
The Dual Decurion's smaller magazine makes it easy to run out of ammo in a gunfight, but its quick ammo recharge balances this out.
A good way to maintain ammunition supply is to fire in bursts of 2-8 bullets.
Its component parts are listed in Offerings as "Decurion" parts, and Dual is not in the parts' name.
**Trivia:**
A Decurion was a Roman cavalry officer originally in command of 10 men.
It is one of several Dual/Akimbo weapons in Warframe with no solo counterpart, including the Twin Gremlins, Akjagara, Akzani, Twin Basolk, Dual Cleavers, Dual Ichor, Dual Zoren.
This also can be applied to Dakra Prime and Dex Dakra which have no default version even though an original version is implied by the name.
When used by Necramechs, instead of dual wielding it only uses a single Decurion.
**Type:** Arch-Gun | **Mastery Req:** 1 | **Riven Disposition:** 4

### Normal Attack
**Damage:** impact: 49.5, slash: 30.25, puncture: 30.25
Crit Chance: 28.00% | Crit Multiplier: 2.2x | Status Chance: 14.00% | Fire Rate: 10.42

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | — |
| Control Module | 2 | — |
| Decurion Barrel | 2 | — |
| Decurion Receiver | 2 | — |

## Fluctus
An Archwing energy weapon that sends waves of deadly plasma crashing into enemies.

### Acquisition
The Fluctus's blueprint can be researched from the Tenno Lab in the dojo. Its parts can be purchased from various Syndicates. All parts, except for the blueprint, are tradeable.

### Wiki
Weapons
An Archwing energy weapon that sends waves of deadly plasma crashing into enemies.
The Fluctus is an Archgun that fires large waves of energy with high punch through, making it excellent at field clearing.
Characteristics
This weapon deals primarily Slash damage.
Innate infinite punch through.
Inflicts a guaranteed Impact proc.
Projectiles have a very wide hitbox.
Projectiles have travel time.
Limited range of 450 meters in Archwing mode.
Limited range of 55 meters in Atmospheric mode.
Innate Naramon polarity.
**Notes:**
The Fluctus' large projectile hitbox makes it a good mid-ranged crowd-control weapon especially when doing Archwing Interception and Defense missions as enemies come in in waves.
The Fluctus' projectiles also bypass the barriers protecting Shield Dargyns (and their Hellion counterparts) from front attacks, giving the weapon an edge over most Archguns when dealing with such units.
This also allows players to safely defeat them from range instead of trying to go into melee, as getting into contact with their shields will cause Electricity damage that can become particularly fatal at higher levels.
**Tips:**
Due to its infinite punch through, the Fluctus is a viable option as a Necramech weapon for Orphix runs as it can easily hit the Resonators that are hiding behind barriers.
**Type:** Arch-Gun | **Mastery Req:** 2 | **Riven Disposition:** 3

### Normal Attack
**Damage:** impact: 50, slash: 175, puncture: 25
Crit Chance: 22.00% | Crit Multiplier: 2x | Status Chance: 16.00% | Fire Rate: 5

### Crafting
| Component | Count | Drops |
|---|---|---|
| Argon Crystal | 2 | — |
| Barrel | 1 | Cephalon Suda, Intelligent (100.00%) |
| Blueprint | 1 | — |
| Limbs | 1 | — |
| Stock | 1 | Steel Meridian, Defender (100.00%) |

## Grattler
Shatter your targets with explosive shells from this devastating Archwing cannon.

### Acquisition
The Grattler's blueprint can be researched from the Chem Lab in the dojo.

### Wiki
Grineer
Shatter your targets with explosive shells from this devastating Archwing cannon.
The Grattler is a quad-barrel Grineer Archgun autocannon. Its explosive projectiles make it ideal for clearing out groups of enemies, which is counterbalanced by its limited range and projectile travel time.
Characteristics
This weapon deals primarily Puncture damage.
Shots explode in a 9 meter radius upon impact with a surface or enemy.
Initial hit and explosion apply status separately.
Explosion does not need direct line of sight to deal damage and will penetrate walls.
Innate Madurai polarity.
Projectiles have travel time.
Requires a spool-up of 4 shots to reach maximum fire rate.
Fire rate starts at 40% of the listed value, and increases by 20% per shot.
Atmospheric:
Linear Damage Falloff from 100% to ?% from 100m to 350m target distance in atmosphere (distances are affected by Projectile Speed).
Projectiles have a maximum range of 225 meters.
Poor ammo efficiency, generating high cooldown for the Archgun Deployer for every second fired.
Explosion inflict self-stagger.
Explosion has a headshot multiplier of 1x and cannot trigger headshot conditions.
Explosion has linear damage falloff from 100% to 30% from central impact.
Kuva Grattler
**Type:** Arch-Gun | **Mastery Req:** 4 | **Riven Disposition:** 3

### Projectile Impact
**Damage:** impact: 15, slash: 15, puncture: 120
Crit Chance: 25.00% | Crit Multiplier: 2x | Status Chance: 25.00% | Fire Rate: 6.25

### Explosion
**Damage:** blast: 205
Crit Chance: 25.00% | Crit Multiplier: 2x | Status Chance: 25.00% | Fire Rate: 6.25

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | — |
| Forma | 1 | — |
| Oxium | 3500 | — |
| Salvage | 6000 | — |
| Tellurium | 5 | — |

## Imperator
A long range rifle capable of firing in space, the Imperator's versatility makes it an ideal weapon for space combat.

### Acquisition
A built Imperator is automatically acquired upon claiming the Odonata Archwing from the Foundry during The Archwing quest.
Additional blueprints can be bought from Cephalon Simaris for 100,000 Standing.

### Wiki
Tenno
A long range rifle capable of firing in space,the Imperator's versatility makes it an ideal weapon for space combat.
The Imperator is a heavy Archgun fully automatic rifle boasting high critical chance, fire rate, and magazine size.
Characteristics
This weapon deals primarily Impact damage.
Innate Madurai polarity.
Good ammo efficiency, generating relatively small cooldown for the Archgun Deployer for every second fired.
Short reload delay of 0.25 seconds in Archwing mode.
Tied with Imperator Vandal for the lowest base damage of all Archguns.
Archwing:
Has linear damage falloff from 100% to ?% from 100m to 350m target distance (distances are affected by Projectile Speed).
Slow ammo regeneration rate relative to magazine size.
Imperator Vandal
**Notes:**
Unlike most primary weapons in the game, Archwing Guns do not have a reserve ammo pool, nor do they need to reload. The Imperator will automatically replenish its magazine's ammo at a fast rate when the weapon is not fired, making it the first weapon in the game that does not require ammo pickups.
Despite its shots being shown as slow and even having bullet drift, the Imperator is a hit-scan weapon, and will immediately hit the enemy within its reticle regardless of distance or the appearance of the bullets it fires. As a side-effect, the Imperator can be used to snipe targets at distances of 2,000 meters or more. However, take note that the Imperator's damage falloff weakens it when attempting to engage enemies from beyond 2,000 meters to the point of dealing no damage per shot.
The Imperator blueprint can be traded between players.
S7oToxPd32g
**Type:** Arch-Gun | **Riven Disposition:** 3

### Normal Attack
**Damage:** impact: 20, slash: 12.5, puncture: 17.5
Crit Chance: 24.00% | Crit Multiplier: 2x | Status Chance: 12.00% | Fire Rate: 16.7

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | Cephalon Simaris, Complete The Archwing (Quest) (100.00%) |
| Circuits | 300 | — |
| Ferrite | 1500 | — |
| Morphics | 2 | — |
| Polymer Bundle | 150 | — |

## Imperator Vandal
The Imperator Vandal has been customized by the Tenno with a blue-green metallic finish and Lotus branding on the barrel.

### Wiki
Weapons
The Imperator Vandal has been customized by the Tenno with a blue-green metallic finish and Lotus branding on the barrel.
The Imperator Vandal is the Vandal form of the Imperator heavy machine Archgun, sporting a higher fire rate, magazine size, and faster ammo regeneration.
Characteristics
This weapon deals primarily Impact damage.
Innate Madurai polarity.
Fastest fire rate of all Archguns once fully spooled up.
Short reload delay of 0.25 seconds from empty, 0.15 seconds when not.
Tied with Mausolon for the largest magazine size of all Archguns.
Tied with Imperator for the lowest base damage of all Archguns.
Requires a spool-up of 16 shots to reach maximum fire rate.
Fire rate starts at 53.33% of the listed value, and increases by 3.11% per shot.
Archwing:
Has linear damage falloff from 100% to ?% from 100m to 350m target distance (distances are affected by Projectile Speed).
Atmosphere:
Sub-par ammo efficiency, generating medium-high cooldown for the Archgun Deployer for every second fired.
Imperator
The Imperator Vandal was awarded during the Operation: Eyes of Blight event for obtaining 15 (PC)/10 (PS4/XB1) Battle Points, complete with a free weapon slot and a pre-installed Orokin Catalyst.
The blueprint and parts are now obtainable from the Balor Fomorian Invasion via Fomorian Sabotage. All components can be traded.
**Notes:**
Has a reload delay of 0.15 seconds while there is still ammo in the magazine. Reload from empty has a delay of 0.25 seconds.
Unlike its standard version, the Imperator Vandal has fire rate spool-up, allowing it to reach a maximum fire rate 50% higher than that of the Imperator. This makes the Imperator Vandal the third Event weapon to have an additional hidden feature over its original version, after Snipetron Vandal and Latron Wraith.
**Type:** Arch-Gun | **Mastery Req:** 5 | **Riven Disposition:** 3

### Normal Attack
**Damage:** impact: 20, slash: 12.5, puncture: 17.5
Crit Chance: 28.00% | Crit Multiplier: 2.4x | Status Chance: 12.00% | Fire Rate: 25

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Fomorian Sabotage (10.00%) |
| Blueprint | 1 | Fomorian Sabotage (10.00%) |
| Receiver | 1 | Fomorian Sabotage (10.00%) |
| Tellurium | 10 | — |

## Kuva Ayanga
Sweep aside hordes of enemies with flaming fury of this powerful, automatic, grenade launching archgun.

### Acquisition
Adversary System/Weapons#Kuva Acquisition

### Wiki
Grineer
Sweep aside hordes of enemies with the flaming fury of this powerful automatic grenade launcher.
The Kuva Ayanga is a unique Kuva Archgun that rapidly fires explosive rounds which travel in an arc.
Characteristics
This weapon deals primarily Blast damage.
Grenades explode after impacting a surface or enemy:
9 meter radius in Archwing mode.
6 meter radius in Atmospheric mode.
Initial hit and explosion apply status separately.
Explosion does not need direct line of sight to deal damage and will penetrate walls.
Adversary Weapon:
Comes with an additional bonus Impact, Heat, Cold, Electricity, Toxin, Magnetic, or Radiation damage, based on the Kuva Lich's progenitor Warframe, which increases the listed base damage of the weapon by 25%-60%.
Valence Fusion can optionally change the resulting output damage type, and can upgrade the bonus increase (up to 60%).
Polarizing the weapon increases its max rank by 2, capping at rank 40 after 5 polarizations, granting the weapon additional mod capacity.
Elemental Vice can be applied to the weapon after it has reached rank 40, allowing the bonus damage type to be swapped freely.
Listed Mastery Rank requirement is not enforced - the weapon may theoretically be obtained at any mastery rank by acquiring and vanquishing a Kuva Lich that has this weapon equipped.
Second slowest fire rate of all Archguns, after Corvas.
Projectiles have travel time with heavy arcing.
Explosion inflicts self-stagger.
Explosion has a headshot multiplier of 1x and cannot trigger headshot conditions.
Explosion has linear Damage Falloff from:
100% to 70% from central impact in Archwing mode.
100% to 50% from central impact in Atmospheric mode.
**Notes:**
Adversary System#Notes
Adversary System/Progenitor
**Type:** Arch-Gun | **Mastery Req:** 13 | **Riven Disposition:** 3

### Grenade Impact
**Damage:** impact: 87
Crit Chance: 35.00% | Crit Multiplier: 2x | Status Chance: 20.00% | Fire Rate: 4.58

### Explosion
**Damage:** blast: 187
Crit Chance: 35.00% | Crit Multiplier: 2x | Status Chance: 20.00% | Fire Rate: 4.58

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | — |

## Kuva Grattler
Reconfigured for maximum lethality, the Kuva Grattler still shatters targets with explosive shells, but now reaches maximum fire-rate instantaneously.

### Wiki
Weapons
Reconfigured for maximum lethality, this powerful Arch-gun still shatters targets with explosive shells, but now reaches its peak fire-rate instantaneously.
The Kuva Grattler is the Kuva variant of the Grattler archgun, with higher critical chance, critical multiplier, and status chance, doubled magazine size, and doubled ammo regeneration in Archwing or Necramech combat, but sacrifices damage and some atmospheric reload speed. It no longer requires spool-up to achieve maximum fire rate, but in turn its base fire rate is slightly lower than a fully spooled normal Grattler.
Characteristics
This weapon does primarily Puncture damage.
Shots explode in a 9 meter radius upon impact with a surface.
Initial hit and explosion apply status separately.
Explosion does not need direct line of sight to deal damage and will penetrate walls.
Innate three Madurai polarities.
Adversary Weapon:
Comes with an additional bonus Impact, Heat, Cold, Electricity, Toxin, Magnetic, or Radiation damage, based on the Kuva Lich's progenitor Warframe, which increases the listed base damage of the weapon by 25%-60%.
Valence Fusion can optionally change the resulting output damage type, and can upgrade the bonus increase (up to 60%).
Polarizing the weapon increases its max rank by 2, capping at rank 40 after 5 polarizations, granting the weapon additional mod capacity.
Elemental Vice can be applied to the weapon after it has reached rank 40, allowing the bonus damage type to be swapped freely.
Listed Mastery Rank requirement is not enforced - the weapon may theoretically be obtained at any mastery rank by acquiring and vanquishing a Kuva Lich that has this weapon equipped.
**Type:** Arch-Gun | **Mastery Req:** 15 | **Riven Disposition:** 1

### Projectile Impact
**Damage:** impact: 9.4, slash: 9.4, puncture: 75.2
Crit Chance: 27.00% | Crit Multiplier: 2.1x | Status Chance: 27.00% | Fire Rate: 5.55

### Explosion
**Damage:** blast: 155
Crit Chance: 27.00% | Crit Multiplier: 2.1x | Status Chance: 27.00% | Fire Rate: 5.55

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | — |

## Larkspur
From death blooms the Larkspur. A unique and menacing Archgun with a wild initial attack that locks onto a target then chains other targets close to the first. It also sports an explosive projectile mode. In Hildryn's grip the Larkspur carries more reserve ammo.

### Wiki
Tenno
From death blooms the Larkspur. A unique and menacing Archgun with a wild initial attack that locks onto a target then chains other targets close to the first. It also sports an explosive projectile mode. In Hildryn's grip the Larkspur carries more reserve ammo.
The Larkspur Archgun is Hildryn's signature heavy weapon, firing a devastating Radiation beam that can chain between targets to hit multiple enemies at once. Its Alternate Fire charges up an explosive Blast round. Holds increased Ammo Maximum when wielded by Hildryn.
Characteristics
This weapon deals primarily Radiation damage.
Primary fire releases a continuous beam that can chain to 3 enemies after hitting the initial target, with each chain doing 80% of the previous chain's damage.
In Atmospheric Mode, the beam will chain to enemies within 6 meters of initial target. In Archwing Mode, the max chain distance is increased to 30 meters.
The main beam will automatically lock onto enemies within 15 degrees of the reticle up to 20 meters away in Atmospheric Mode. In Archwing Mode, the max lock-on distance is increased to 160 meters.
Enemy corpses will still allow the beam to connect to nearby enemies.
Consumes 0.5 ammo per tick of damage.
No Damage Falloff in Archwing Mode.
Alternate Fire charges and releases an energy grenade that explodes in an 9.6 meter radius upon impacting a surface.
Pinpoint accuracy.
Projectile has guaranteed knockdown proc.
Projectile has infinite punch through on bodies.
Initial hit and explosion apply status separately.
Explosion does not need direct line of sight to deal damage and will penetrate walls.
Innate Madurai polarity.
Primary Fire is tied with Mausolon alt-fire, Arquebex, and Morgha alt-fire for the highest status chance of all Archguns.
Alt-fire has good ammo efficiency in Atmospheric Mode, generating relatively low cooldown for the Archgun Deployer for every second fired.
**Type:** Arch-Gun | **Mastery Req:** 8 | **Riven Disposition:** 3

### Normal Attack
**Damage:** impact: 10, radiation: 80
Crit Chance: 10.00% | Crit Multiplier: 1.4x | Status Chance: 50.00% | Fire Rate: 12

### Alt-Fire Projectile Impact
**Damage:** impact: 140, radiation: 100, blast: 180
Crit Chance: 26.00% | Crit Multiplier: 2.2x | Status Chance: 34.00% | Fire Rate: 1

### Explosion
**Damage:** radiation: 400, blast: 400
Crit Chance: 26.00% | Crit Multiplier: 2.2x | Status Chance: 34.00% | Fire Rate: 1

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | — |
| Crisma Toroid | 2 | — |
| Radiant Zodian | 5 | — |
| Synathid Ecosynth Analyzer | 5 | — |
| Venerdo Alloy | 80 | — |

## Larkspur Prime
Embrace pandemonium. Chainfire and explosive potential meet in Larkspur Prime. Hildryn packs more ammo when she wields this mighty weapon.

### Wiki
Weapon
Embrace pandemonium. Chainfire and explosive potential meet in Larkspur Prime. Hildryn packs more ammo when she wields this mighty weapon.
Larkspur Prime is the Primed counterpart of the Larkspur heavy Archgun, sporting increased Critical Chance and Critical Multiplier, and Status Chance on Alternate Fire. Larkspur Prime was released alongside Hildryn Prime and Shade Prime.
Characteristics
Conflicting information on ammo efficiency and Archgun Deployer cooldown
This weapon deals primarily Radiation damage.
Primary fire releases a continuous beam that can chain to 3 enemies after hitting the initial target, with each chain doing 80% of the previous chain's damage.
In Atmospheric Mode, the beam will chain to enemies within 6 meters of initial target. In Archwing Mode, the max chain distance is increased to 30 meters.
The main beam will automatically lock onto enemies within 20 degrees of the reticle up to 30 meters away in Atmospheric Mode. In Archwing Mode, the max lock-on distance is increased to 180 meters.
Enemy corpses will still allow the beam to connect to nearby enemies.
Consumes 0.5 ammo per tick of damage.
No Damage Falloff in Archwing Mode.
Alternate Fire charges and releases an energy grenade that explodes in an 9.6 meter radius upon impacting a surface.
Pinpoint accuracy.
Projectile has forced Knockdown status effect.
Projectile has infinite punch through on bodies.
Initial hit and explosion apply status separately.
Explosion does not need direct line of sight to deal damage and will penetrate walls.
Innate Madurai polarity.
Primary Fire is tied with Mausolon alt-fire, Arquebex, and Morgha alt-fire for the highest status chance of all Archguns.
Good ammo efficiency in Atmospheric Mode, generating relatively low cooldown for the Archgun Deployer for every second fired.
**Type:** Arch-Gun | **Mastery Req:** 8 | **Riven Disposition:** 2

### Normal Attack
**Damage:** impact: 10, radiation: 80
Crit Chance: 14.00% | Crit Multiplier: 1.6x | Status Chance: 50.00% | Fire Rate: 12

### Alt-Fire Projectile Impact
**Damage:** impact: 140, radiation: 100, blast: 180
Crit Chance: 28.00% | Crit Multiplier: 2.2x | Status Chance: 38.00% | Fire Rate: 2

### Explosion
**Damage:** radiation: 400, blast: 400
Crit Chance: 28.00% | Crit Multiplier: 2.2x | Status Chance: 38.00% | Fire Rate: 2

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Axi L6 Relic (2.00%); Lith L6 Relic (2.00%); Neo L2 Relic (2.00%); Neo L3 Relic (2.00%); Neo L4 Relic (2.00%) |
| Blueprint | 1 | Axi S16 Relic (25.33%); Lith T12 Relic (25.33%); Lith X1 Relic (25.33%); Meso B9 Relic (25.33%); Meso H4 Relic (25.33%); Meso K6 Relic (25.33%); Meso M4 Relic (25.33%); Neo H4 Relic (25.33%) |
| Orokin Cell | 10 | — |
| Receiver | 1 | Axi A19 Relic (11.00%); Axi G10 Relic (11.00%); Lith G12 Relic (11.00%); Lith P9 Relic (11.00%); Meso G10 Relic (11.00%); Neo D8 Relic (11.00%); Neo F3 Relic (11.00%) |
| Stock | 1 | Axi D5 Relic (11.00%); Lith F3 Relic (11.00%); Lith K10 Relic (11.00%); Lith W3 Relic (11.00%); Meso W3 Relic (11.00%); Neo A12 Relic (11.00%); Neo P7 Relic (11.00%) |

## Mandonel
Mandonel fires Radiation Damage projectiles. Partially charged shots release a spread and fully charged shots release a beam that dissolves into a radiation field. Projectiles that pass through the field are empowered.

### Wiki
Orokin
Mandonel fires Radiation Damage projectiles. Partially charged shots release a spread and fully charged shots release a beam that dissolves into a radiation field. Projectiles that pass through the field are empowered.
The Mandonel Archgun is Qorvex's signature heavy weapon, sporting his brutalist concrete design while also having Entrati ornamentation and gilding. This weapon fires Radiation shots with a large horizontal spread and can be fully charged to unleash a beam that leaves behind a trail that strengthens projectiles that pass through it. The fully charged beam can empower Chyrinka Pillar.
Characteristics
This weapon deals primarily Radiation damage.
Uncharged shots fire projectiles with a wide horizontal spread based on charge.
Spread is reduced the longer the charge is held.
Fully charged shots fire a laser that leaves behind a 8 second radiation field. Shots passing through the field gain increased damage, Status Chance, Critical Chance, Punch Through, and Projectile Speed, and can Ricochet off surfaces.
Mandonel's main blueprint is awarded from Sanctum Anatomica Bounties as a rare drop. The Barrel, Receiver, and Stock are awarded from Albrecht's Laboratories Mirror Defense (Munio), Survival (Persto), and Alchemy (Cambire) endless missions on Deimos as a Rotation B and C drop.
**Type:** Arch-Gun | **Mastery Req:** 10

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Deimos/Munio (Defense), Rotation C (15.00%); Deimos/Cambire (Alchemy), Rotation B (17.50%); Deimos/Persto (Survival), Rotation C (25.00%) |
| Blueprint | 1 | Deimos/Albrecht's Laboratories (Level 65 - 70 Entrati Lab Bounty), Rotation C (3.39%); Deimos/Albrecht's Laboratories (Level 75 - 80 Entrati Lab Bounty), Rotation C (4.13%); Deimos/Albrecht's Laboratories (Level 115 - 120 Entrati Lab Bounty), Rotation C (4.20%); Deimos/Albrecht's Laboratories (Level 55 - 60 Entrati Lab Bounty), Rotation C (4.35%); Deimos/Albrecht's Laboratories (Level 95 - 100 Entrati Lab Bounty), Rotation C (4.39%) |
| Echo Voca | 2 | — |
| Receiver | 1 | Deimos/Cambire (Alchemy), Rotation C (16.00%); Deimos/Munio (Defense), Rotation B (17.50%); Deimos/Persto (Survival), Rotation C (25.00%) |
| Stock | 1 | Deimos/Munio (Defense), Rotation C (15.00%); Deimos/Cambire (Alchemy), Rotation C (16.00%); Deimos/Persto (Survival), Rotation B (17.50%) |

## Mausolon
An ancient weapon designed by the Entrati for use by their Necramechs. Primary fire siphons life essence from the target to fuel a devastating secondary fire. Punishing automatic primary fire and a secondary mode that charges up to unleash a destructive beam of energy with a large explosion at point of impact.

### Wiki
Weapons
An ancient weapon designed by the Entrati for use by their Necramechs. Primary fire siphons life essence from the target to fuel a devastating alt fire. Punishing automatic primary fire and an alt mode that charges up to unleash a destructive beam of energy with a large explosion at point of impact.
The Mausolon is an ancient Orokin fully automatic heavy rifle designed by the Entrati, serving as the primary armament for the Necramechs. It can be used by Warframes as an Archgun.
Boasting high critical chance, status chance, accuracy, magazine size, and fire rate, the primary fire shoots fully automatic explosive rounds that builds charges on kills to allow use of the Alternate Fire, an obliterating explosive laser.
Characteristics
This weapon deals primarily Heat and Puncture damage.
Primary fire shoots fully automatic rounds.
Shots explode in a 1.8 meter radius on impact with a surface or enemy.
Damaging Lifted enemies causes up to 13 additional instances of direct hit damage.
Getting 5 kills with the Mausolon's primary fire will unlock an Alternate Fire that discharges a powerful laser that explodes on impact.
Shots explode in a 8 meter radius on impact with a surface or enemy.
Guaranteed Lifted proc.
After using Alternate Fire, additional kills are needed to recharge the laser.
Initial hits and explosions apply status separately.
Explosions do not need direct line of sight to deal damage and will penetrate walls.
Innate Madurai polarity.
Alternate Fire:
Tied with Arquebex for the second highest critical chance of all Archguns, behind Velocitus.
Second highest critical multiplier of all Archguns, behind Velocitus.
Tied with Larkspur primary fire, Arquebex, and Morgha alt-fire for the highest status chance of all Archguns.
Pinpoint accuracy.
Fast charge speed.
Does not use ammo.
Tied with Imperator Vandal for the largest magazine size of all Archguns.
**Type:** Arch-Gun | **Mastery Req:** 14 | **Riven Disposition:** 1

### Auto
**Damage:** impact: 35, puncture: 70, heat: 75
Crit Chance: 30.00% | Crit Multiplier: 2.2x | Status Chance: 26.00% | Fire Rate: 8.33

### Auto Radial Attack
**Damage:** heat: 72
Crit Chance: 30.00% | Crit Multiplier: 2.2x | Status Chance: 26.00% | Fire Rate: 8.33

### Charged Shot Laser
**Damage:** impact: 150, puncture: 600, heat: 750
Crit Chance: 50.00% | Crit Multiplier: 3.5x | Status Chance: 50.00% | Fire Rate: 0.5

### Charged Shot Explosion
**Damage:** heat: 4500
Crit Chance: 50.00% | Crit Multiplier: 3.5x | Status Chance: 50.00% | Fire Rate: 0.5

## Morgha
Blast through enemies with a double-barreled shot of energized slugs. This siphons their life essence which is then used to generate the secondary fire's massive air-burst mortar. The ancient Entrati weapon was built for Necramechs but also functions as an Archgun.

### Wiki
Orokin
Blast through enemies with a double-barreled shot of energized slugs. This siphons their life essence which is then used to generate the secondary fire’s massive air-burst mortar. The ancient Entrati weapon was built for Necramechs but also functions as an Archgun.
The Morgha is an Orokin Era weapon created by the Entrati, a grenade launcher Archgun that builds charges on kills to allow use of the Alternate Fire, an air-burst mortar.
Characteristics
This weapon deals primarily Blast damage.
Primary fire is a fully automatic 2-round burst grenade launcher.
Shots explode in a 3 meter radius on impact with a surface or enemy.
Getting 5 kills with the Morgha's primary fire will unlock an Alternate Fire that releases a single mortar that bounces on surfaces or enemies and exploding in a 20 meter (12 meters in atmospheric mode) radius after traveling 14 meters in midair (distance is affected by Projectile Speed).
Pinpoint accuracy.
After using Alternate Fire, additional kills are needed to recharge the mortar.
Initial hit and explosions apply status separately.
All explosions do not need direct line of sight to deal damage and will penetrate walls.
Innate Madurai polarity.
Alternate Fire is tied with Larkspur primary fire, Mausolon alt-fire, and Arquebex for the highest status chance of all Archguns.
Primary Fire:
Explosion has linear Damage Falloff from 100% to 70% from central impact.
Alternate Fire:
Requires 5 primary fire kills to charge a gauge and cannot fire without a full gauge.
Extremely slow Projectile Speed and needs to travel 14m before being explosive.
All explosions inflict self-stagger.
All explosions have a headshot multiplier of 1x and cannot trigger headshot conditions.
All projectiles have travel time.
Slow reload speed.
Atmospheric:
Poor ammo efficiency, generating high cooldown for the Archgun Deployer for every second fired, and requires just 2 reloads to fully deplete all ammo reserves.
**Type:** Arch-Gun | **Mastery Req:** 15 | **Riven Disposition:** 3

### Auto-Burst
**Damage:** impact: 32
Crit Chance: 30.00% | Crit Multiplier: 2.2x | Status Chance: 28.00% | Fire Rate: 3

### Auto Burst Explosion
**Damage:** blast: 164
Crit Chance: 30.00% | Crit Multiplier: 2.2x | Status Chance: 28.00% | Fire Rate: 3

### Charged Shot
**Damage:** impact: 100
Crit Chance: 40.00% | Crit Multiplier: 3x | Status Chance: 50.00% | Fire Rate: 0.833

### Charged Shot Explosion
**Damage:** impact: 600, slash: 1000, puncture: 800, blast: 1200
Crit Chance: 40.00% | Crit Multiplier: 3x | Status Chance: 50.00% | Fire Rate: 0.833

### Crafting
| Component | Count | Drops |
|---|---|---|
| Blueprint | 1 | NecraLoid (Loid), Clearance Odima (100.00%) |
| Morgha Barrel | 1 | — |
| Morgha Receiver | 1 | — |
| Morgha Stock | 1 | — |

## Phaedra
Devastate free-space enemies with Phaedra, the Soma's big-sister.

### Acquisition
The Phaedra's blueprint can be purchased from the Market. Its parts can be purchased from various Syndicates, with the receiver also being rare drop from Archwing Exterminate on Montes, Venus and Syrtis, Mars. All parts, except for the blueprint, are tradeable.

### Wiki
Tenno
Devastate free-space enemies with Phaedra, the Soma's big-sister.
The Phaedra is a large-caliber Archgun that shares design aesthetics and mechanics with the Soma.
Characteristics
This weapon deals primarily Puncture damage.
Innate Madurai polarity.
Very high fire rate once fully spooled.
Short reload delay of 0.25 seconds in Archwing mode.
Good ammo efficiency, generating relatively small cooldown for the Archgun Deployer for every second fired.
Archwing:
Has linear damage falloff from 100% to ?% from 100m to 350m target distance (distances are affected by Projectile Speed).
Slow ammo regeneration rate relative to magazine size.
Requires a spool-up of 11 shots before optimal fire rate is achieved.
Fire rate starts at 50% of the listed value, and increases by 5% per shot.
**Notes:**
In contrast to the Soma, Phaedra has stats that favor status chance, instead of utilizing critical chance and critical multiplier, a trait the Soma is known for.
**Trivia:**
Phaedra is a character in Greek Mythology best known for her role in Euripides' play, Hippolytus. Phaedra's name derives from the Greek word φαιδρός (phaidros), which means "bright".
**Type:** Arch-Gun | **Mastery Req:** 3 | **Riven Disposition:** 4

### Normal Attack
**Damage:** impact: 14, slash: 5.6, puncture: 36.4
Crit Chance: 14.00% | Crit Multiplier: 2x | Status Chance: 30.00% | Fire Rate: 18.75

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Arbiters of Hexis, Authentic (100.00%) |
| Blueprint | 1 | — |
| Receiver | 1 | The Perrin Sequence, Senior Associate (100.00%); Mars/Syrtis (Exterminate) (5.64%); Mercury/Caduceus (Exterminate) (5.64%); Venus/Montes (Exterminate) (5.64%) |
| Stock | 1 | New Loka, Bountiful (100.00%) |
| Tellurium | 2 | — |

## Prisma Dual Decurions
Colder than space and less forgiving, Prisma crystals bring elite efficiency to the already sleek Dual Decurion. Requiring the lightest of touches to reload, these twin archguns prove that heavy weaponry can still possess elegance.

### Acquisition
The Prisma Dual Decurions is potentially sold by Baro Ki'Teer in the Concourse section of the Tenno Relay for 175,000 and 525. Note that Baro Ki'Teer's stock changes with each appearance, and this weapon may not be available on a succeeding appearance.
The weapon can be traded, but only for unranked copies without Forma and Orokin Catalyst installed.

### Wiki
Tenno
Colder than space and less forgiving, Prisma crystals bring elite efficiency to the already sleek Dual Decurion. Requiring the lightest of touches to reload, these twin archguns prove that heavy weaponry can still possess elegance.
The Prisma Dual Decurions is the Prisma variant of the Dual Decurion twin Archgun rifles, boasting higher critical chance, fire rate, status chance, and accuracy.
Characteristics
This weapon deals primarily Impact damage.
Innate two Madurai polarities.
Good ammo efficiency, generating relatively small cooldown for the Archgun Deployer for every second fired.
Short reload delay of 0.25 seconds in Archwing mode.
Tied with Dual Decurion for the second fastest reload speed of all Archguns in Atmospheric mode, behind Cyngas.
Has notable recoil which throws off accuracy at medium-long range shooting.
Dual Decurion
**Trivia:**
When used by Necramechs, instead of dual wielding it only uses a single Decurion.
**Type:** Arch-Gun | **Mastery Req:** 1 | **Riven Disposition:** 4

### Normal Attack
**Damage:** impact: 49.5, slash: 30.25, puncture: 30.25
Crit Chance: 30.00% | Crit Multiplier: 2.2x | Status Chance: 24.00% | Fire Rate: 12.5

## Velocitus
When fully charged, the magnetized barrel of the Velocitus accelerates a metal slug to tremendous speeds, piercing hulls and obliterating armor.

### Acquisition
The Velocitus's blueprint can be researched from the Tenno Lab in the dojo. Its parts can be purchased from various Syndicates. All parts, except for the blueprint, are tradeable.

### Wiki
Weapons
When fully charged, the magnetized barrel of the Velocitus accelerates a metal slug to tremendous speeds, piercing hulls and obliterating armor.
The Velocitus is an Archgun chargeable gauss rifle that inflicts high damage, equally split between Impact, Puncture, Slash, and Magnetic damage.
Characteristics
This weapon deals equal physical and Magnetic damage.
Highest base damage of Archguns.
Fully charged shots have the highest critical multiplier of all Archguns.
Innate punch through:
Infinite against bodies in Archwing Mode.
5 meters in Atmospheric Mode.
Projectiles ricochet off of walls in Archwing Mode, making it ideal for combat in closed spaces.
High accuracy.
Good ammo efficiency when used charged, generating relatively small cooldown for the Archgun Deployer for every second fired.
No damage falloff in Archwing Mode.
Projectiles have travel time.
Shots that are not fully charged will always deal the same damage no matter how close to being fully charged.
Atmosphere:
Fully charged shots cause a heavy recoiling effect that breaks zoom.
Terrible ammo efficiency when spammed uncharged, generating very high cooldown for the Archgun Deployer for every second fired.
**Type:** Arch-Gun | **Riven Disposition:** 3

### Uncharged Shot
**Damage:** impact: 150, slash: 150, puncture: 150, magnetic: 150
Crit Chance: 30.00% | Crit Multiplier: 2x | Status Chance: 25.00% | Fire Rate: 5

### Charged Shot
**Damage:** impact: 400, slash: 400, puncture: 400, magnetic: 400
Crit Chance: 60.00% | Crit Multiplier: 3.6x | Status Chance: 25.00% | Fire Rate: 5

### Crafting
| Component | Count | Drops |
|---|---|---|
| Barrel | 1 | Cephalon Suda, Intriguing (100.00%) |
| Blueprint | 1 | — |
| Control Module | 2 | — |
| Receiver | 1 | Steel Meridian, Valiant (100.00%) |
| Stock | 1 | Red Veil, Honored (100.00%) |
